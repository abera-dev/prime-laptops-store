            ...
            <div key={item.id} className="card p-4 flex gap-4 items-center">
              ...
              <div className="flex items-center border border-white/10 rounded-lg overflow-hidden bg-white/5">
                <button onClick={() => updateItem(item.id, item.quantity - 1)} disabled={item.quantity <= 1}
                  className="px-2.5 py-1.5 hover:bg-white/10 text-sm font-bold disabled:opacity-40">-</button>
              ...
              <p className="tech-price w-20 text-right">${(item.price * item.quantity).toLocaleString()}</p>
              <button onClick={() => removeItem(item.id)} className="text-rose-400 hover:text-rose-300 ml-1">
